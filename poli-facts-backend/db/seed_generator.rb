...
def createPoliticians(pol, newStateId)
...
def make_state_tables()
  ...
  senateMembers = senateCongress["results"][0]["members"]
  houseResponse = RestClient.get "https://api.propublica.org/congress/v1/115/house/members", headers= {"X-API-KEY":"Jv6lbl95ZWrgwK8bThHUaXcaVfojPdqG6V7rR3QY"}
  houseCongress = JSON.parse(houseResponse)
  houseMembers = houseCongress["results"][0]["members"]
  ...
  @states_array.each do |state|  
    ...
    stateHash = {
      ...
      abbrv: state[:abbrv], 
      census_id: state[:state_id], 
      poverty: state[:Poverty], 
      high_school_grad: state[:high_school],
      bachelor_deg: state[:bach_degree],
      white: state[:White],
      black: state[:Black],
    ...
    returnedHouse = houseMembers.select {|member| member["state"] == newState.abbrv}

    returnedSenate.each do |member| 
      if member["in_office"]
          createPoliticians(member, newState.id)
      end
    end

    returnedHouse.each do |member| 
      if member["in_office"]
          createPoliticians(member, newState.id)
      end
    end
